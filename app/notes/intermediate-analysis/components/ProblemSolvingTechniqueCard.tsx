...
interface ProblemSolvingTechniqueCardProps {
  number: number;
  title: string;
  description?: string;
  examples?: Example[];
...
export default function ProblemSolvingTechniqueCard({ number, title, description, examples = [] }: ProblemSolvingTechniqueCardProps) {
  ...
  return (
    <div className="border border-gray-300 rounded-lg p-3 sm:p-4 mb-3 sm:mb-4 bg-white shadow-sm">
      <div className="mb-2 sm:mb-3 text-sm sm:text-base">
        <span className="font-bold text-blue-600">Technique {number}. </span>
        <span className="font-semibold text-black">{title}</span>
      </div>

      {description && (
        <div className="text-black text-sm sm:text-base ml-0 sm:ml-[7em] mb-4">
          <p>{description}</p>
        </div>
      )}
      ...
      {examples.length > 0 && (
        <div className="mt-4 space-y-3">
          <div className="font-semibold text-gray-700 text-sm sm:text-base ml-0 sm:ml-[7em]">
            Examples:
          </div>
          {examples.map((example, index) => {
            const isOpen = openExamples.has(index);
            return (
              <div key={index} className="ml-0 sm:ml-[7em] border border-gray-200 rounded-lg p-3 sm:p-4 bg-gray-50">
                <div
